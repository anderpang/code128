   ...
   (function(ctx,factory){
       ...
       function code128(input,option){
           ...
           //code128C
           if(/^\d+$/.test(input)){
               result[0]=codingTable[105];
               //如果奇数长
               if(len%2){
                   trace(result,input,len-1,2,3);

                   //加上A编码 
                   result.push(codingTable[101]);
                   //找最后一位编码
                   result.push(findChar(input[len-1],1));
               }
               else{
                   trace(result,input,len,2,3);
               }
           }
           //code128B
           else if(/[a-z]/.test(input)){
               result[0]=codingTable[104];
               trace(result,input,len,1,2);
           }
           //code128A
           else{
               result[0]=codingTable[103];
               trace(result,input,len,1,1);
           }

           //计算检验位
           check(result);

           result.push(codingTable[106]);

           result=result.map(function(item){return item[5]}).join("");

           return option==="DOM"?createDom(result):result;
           
       }

       function trace(result,input,len,step,codeIndex){
           var i=0;
           for(;i<len;i+=step){
               result.push(findChar(input.substr(i,step),codeIndex));
           }
       }

       function findChar(char,codeIndex){
           var i=codingTable.length;
           while(i--){
              if(codingTable[i][codeIndex]===char){
                  return codingTable[i];
              }
           }
           throw new Error("Not found char:"+char);
       }
       ...
       function check(result){
           ...
           result.push(codingTable[sum%103]);
       }
       ...
       function createDom(code){
            var i=0,
                ...
                html="";
            for(;i<ii;i++){
               html+=code[i]==="1"?'<div class="b"></div>':'<div class="s"></div>';
            }
